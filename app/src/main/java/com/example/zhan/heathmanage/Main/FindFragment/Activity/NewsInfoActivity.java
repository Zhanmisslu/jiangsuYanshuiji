...
public class NewsInfoActivity extends BaseActivity {
    //@BindView(R.id.newsinfo_web)
    WebView newsinfo_web;
    String url="";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        newsinfo_web=findViewById(R.id.newsinfo_web);
        newsinfo_web.loadUrl(url);
        newsinfo_web.addJavascriptInterface(this,"android");
        newsinfo_web.setWebChromeClient(webChromeClient);
        newsinfo_web.setWebViewClient(webViewClient);
        WebSettings webSettings = newsinfo_web.getSettings();
...
//                                .setDurationTime(0.5) // 设置动画时间百分比
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            ...
            if(url.equals("http://www.google.com/")){
                Toast.makeText(NewsInfoActivity.this,"国内不能访问,拦截该url",Toast.LENGTH_LONG).show();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        Log.i("ansen","是否有上一个页面:"+newsinfo_web.canGoBack());
        if (newsinfo_web.canGoBack() && keyCode == KeyEvent.KEYCODE_BACK){//点击返回按钮的时候判断有没有上一页
            newsinfo_web.goBack(); // goBack()表示返回webView的上一页面
            return true;
        }
