package com.example.zhan.heathmanage.Main.TrendFragment.manage;
...
import com.example.zhan.heathmanage.Main.TrendFragment.View.LineChartMarkView;
import com.example.zhan.heathmanage.Main.TrendFragment.util.DateUtil;
...
//        lineDataSet.setCircleColor(R.color.green);
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<MonthInfo> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            MonthInfo data = dataList.get(i);
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i,Float.valueOf(data.getSystolicBP()));
            entries.add(entry);
        }
...
//        leftYAxis.setZeroLineColor(Color.GRAY);
        ...
        leftYAxis.setValueFormatter(new IAxisValueFormatter() {
            ...
            //y轴坐标
            public String getFormattedValue(float value, AxisBase axis) {
                Log.v("zjc", String.valueOf(value));
        ...
        lineDataSet.setColor(R.color.color1);
    ...
    public void showBloodOxygenLineChart(final List<MonthInfo> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            MonthInfo data = dataList.get(i);
            /**
             ...
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i,Float.valueOf(data.getBloodOxygen()));
            entries.add(entry);
        }
...
//    public void showBloodFatLineChart(final List<MonthInfo> dataList, String name, int color) {
//        List<Entry> entries = new ArrayList<>();
//        for (int i = 0; i < dataList.size(); i++) {
//            MonthInfo data = dataList.get(i);
//            /**
...
//             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
//             */
//            Entry entry = new Entry(i,Float.valueOf(data.getBloodFat()));
//            entries.add(entry);
//        }
//
//        /******根据需求的不同 在此在次设置X Y轴的显示内容******/
...
//        lineChart.setData(lineData);
//    }
    public void showHeartRateLineChart(final List<MonthInfo> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            MonthInfo data = dataList.get(i);
            /**
             ...
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i,Float.valueOf(data.getHeartRate()));
            entries.add(entry);
        }
...
//        leftYAxis.setZeroLineColor(Color.GRAY);
     ...
     * 添加曲线
     */
    public void addLine(List<MonthInfo> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            MonthInfo data = dataList.get(i);
            Entry entry = new Entry(i, Float.valueOf(data.getDiastolicBP()) );
            entries.add(entry);
        }
        // 每一个LineDataSet代表一条线
        LineDataSet lineDataSet = new LineDataSet(entries, name);
